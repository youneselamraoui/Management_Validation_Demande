...
const ValidationFinance = () => {
    ...
    const load = async () => {
      try {
        ...
        const [resDemandes, resCapex] = await Promise.all([
          axios.get("http://localhost:5056/api/demandes/finance", { headers }),
  ...
  const handleConfirm = async () => {
    try {
      ...
      setSnackbar({
        open: true,
        message: actionType === "valider" ? "Request sent to Director ✅" : "Request rejected ❌",
    ...
    <Sidebar>
      <Box sx={{ p: 3 }}>
        ...
        <Typography variant="body2" color="text.secondary" mb={3}>
          {demandes.length} request(s) pending finance confirmation
        </Typography>
        ...
        <Dialog open={confirmOpen} onClose={() => setConfirmOpen(false)} maxWidth="xs" fullWidth>
          ...
          <DialogContent>
            <Typography variant="body2" color="text.secondary" mb={2}>
              {actionType === "valider"
                ? "The request will be sent to the Director for final approval."
...
export default ValidationFinance;
