...
export default function GestionFournisseurs() {
  ...
  const [filteredFournisseurs, setFilteredFournisseurs] = React.useState([]);
    ...
    { field: 'active', headerName: 'Actif', width: 100 },
    {
      ...
      renderCell: (params) => (
        <>
          <IconButton
            color="primary"
            onClick={() => navigate(`/settings/fournisseurs/edit/${params.row.id}`)}
          >
            <EditIcon />
          </IconButton>
          <IconButton
  ...
  React.useEffect(() => {
    const fetchFournisseurs = async () => {
      try {
        ...
        const response = await axios.get("http://localhost:5056/api/fournisseurs", {
  ...
  const handleAddClick = () => navigate('/settings/fournisseurs/add');
  ...
  const handleConfirmDelete = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.delete(`http://localhost:5056/api/fournisseurs/${deleteId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
